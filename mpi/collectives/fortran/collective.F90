program coll_exer
  ...
  arr = (/ 1,1,2,4 /)
  call init_buffers
  call mpi_gatherv(sendbuf, arr(rank+1), MPI_INTEGER, recvbuf, arr, (/ 0,1,2,4 /), MPI_INTEGER, 1, &
                   MPI_COMM_WORLD, ierr)
  call print_buffers(recvbuf)

  arr = (/ 2,2,2,2 /)
  !do i = 0, 3
  !  call mpi_scatterv(sendbuf, arr, arr*i, MPI_INTEGER, &
  !                    recvbuf, arr(rank+1), MPI_INTEGER, i, &
  !                    MPI_COMM_WORLD, ierr)
  !end do
  call init_buffers
