program coll_exer
  ...
  if (rank < ntasks/2) then
    color = 1
  else
    color = 2
  end if

  call mpi_comm_split(MPI_COMM_WORLD, color, rank, half_half_comm)
  ...
  call mpi_reduce(sendbuf, recvbuf, 8, MPI_INTEGER, MPI_SUM, 0, half_half_comm)
